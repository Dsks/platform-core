 ...
 *
 * <p>Registration intentionally keeps new accounts and existing unverified accounts on the same
 * verification-required outcome. Existing verified users receive an already-registered outcome so
 * clients can route them to login. Raw passwords, OTPs, verification sessions, and other secrets
 * must not be logged in clear text by callers or adapters.
 */
public class RegisterUserService implements RegisterUserUseCase {
   ...
   * without changing the password. For an existing verified user, it returns an already-registered
   * result without a verification session.
   *
    ...
    if (existing.isPresent()) {
      if (!existing.get().isVerified()) {
        ...
        return new Result(
            ...
            issueResult.verificationSessionId(),
            issueResult.ttlSeconds());
      }
      return new Result(requestId, RegistrationStatus.ALREADY_REGISTERED, null, 0);
    }
