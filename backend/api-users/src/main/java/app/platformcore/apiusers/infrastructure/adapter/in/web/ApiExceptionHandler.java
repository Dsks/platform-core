...
public class ApiExceptionHandler {
   ...
   * @return problem detail with PlatformCore problem type and sanitized params where required
   */
  ...
  /**
   * Creates the shared problem-detail shape with the PlatformCore problem URI used by web clients.
   */
  private ProblemDetail buildProblem(HttpStatus status, String code, String detail) {
    var pd = ProblemDetail.forStatusAndDetail(status, detail);
    pd.setTitle(code);
    pd.setType(URI.create("https://platformcore.app/problems/" + code));
    return pd;
  }
