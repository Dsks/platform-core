...
public class SecurityConfig {
   ...
   *
   * @param csrfCookieSecure value of {@code platformcore.security.csrf.cookie.secure}; defaults to
   *     {@code true}
   * @param csrfCookieSameSite value of {@code platformcore.security.csrf.cookie.same-site};
   *     defaults to {@code Lax}
   */
  public SecurityConfig(
      @Value("${platformcore.security.csrf.cookie.secure:true}") boolean csrfCookieSecure,
      @Value("${platformcore.security.csrf.cookie.same-site:Lax}") String csrfCookieSameSite) {
   ...
   * authenticated, administrative users routes are limited to ADMIN and SUPERADMIN roles, remaining
   * users routes require authentication, and every other route is protected by default. The JWT
  ...
  private void writeForbiddenProblem(HttpServletResponse response) throws IOException {
    ...
    response
        .getWriter()
        .write(
            "{\"type\":\"https://platformcore.app/problems/FORBIDDEN_OPERATION\","
