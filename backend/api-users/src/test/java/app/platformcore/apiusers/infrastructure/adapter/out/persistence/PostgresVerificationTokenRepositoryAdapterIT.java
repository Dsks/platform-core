...
import app.platformcore.apiusers.domain.model.VerificationTokenId;
...
class PostgresVerificationTokenRepositoryAdapterIT {
  ...
  @Autowired private VerificationTokenRepositoryPort tokenRepository;
  ...
  void save_shouldPersistTokenAndMapNullableColumnsWhenReadingLatest() {
    ...
    VerificationToken stored =
        tokenRepository
            .findLatestByUserAndType(userId, VerificationToken.Type.EMAIL_VERIFICATION)
            .orElseThrow();
  ...
  void findLatestActiveByUserId_shouldReturnNewestTokenThatIsNotConsumedAndNotExpired() {
    ...
    VerificationToken newestActive =
        newToken(
            userId,
    ...
    VerificationToken found =
        tokenRepository
            .findLatestActiveByUserId(userId, VerificationToken.Type.EMAIL_VERIFICATION, now)
            .orElseThrow();
  ...
  void findActiveBySessionAndType_shouldIgnoreExpiredAndConsumedTokens() {
    ...
    assertThat(
            tokenRepository.findActiveBySessionAndType(
                activeSession, VerificationToken.Type.EMAIL_VERIFICATION, now))
        .isPresent();
    assertThat(
            tokenRepository.findActiveBySessionAndType(
                expiredSession, VerificationToken.Type.PASS_RESET, now))
        .isEmpty();
    assertThat(
            tokenRepository.findActiveBySessionAndType(
                consumedSession, VerificationToken.Type.PASS_RESET, now))
        .isEmpty();
  }
