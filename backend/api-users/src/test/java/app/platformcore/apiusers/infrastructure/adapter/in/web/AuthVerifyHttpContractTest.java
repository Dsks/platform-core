...
import app.platformcore.apiusers.application.port.in.RegisterUserUseCase.RegistrationStatus;
...
@TestPropertySource(
    properties = {
      "platformcore.security.cookie.secure=true",
      "platformcore.security.cookie.same-site=Strict",
      "platformcore.security.verification.cookie.name=PLATFORMCORE_VERIF_TEST",
      "platformcore.security.verification.cookie.secure=true",
      "platformcore.security.verification.cookie.same-site=None",
      "platformcore.security.jwt.expiration-ms=120000"
    })
class AuthVerifyHttpContractTest {
  ...
  void login_shouldReturn204AndEmitAuthCookie_whenCredentialsAreValid() throws Exception {
    ...
    Assertions.assertTrue(setCookie.contains("PLATFORMCORE_AUTH=jwt-token-123"));
  ...
  void login_shouldReturnUnauthorizedProblemDetail_whenCredentialsAreInvalid() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.type").value("https://platformcore.app/problems/INVALID_CREDENTIALS"))
  ...
  void logout_shouldReturn204AndExpireAuthCookie() throws Exception {
    ...
    String setCookie = response.getHeader(HttpHeaders.SET_COOKIE);
    Assertions.assertNotNull(setCookie);
    Assertions.assertTrue(setCookie.contains("PLATFORMCORE_AUTH="));
    Assertions.assertTrue(setCookie.contains("HttpOnly"));
    Assertions.assertTrue(setCookie.contains("Secure"));
    Assertions.assertTrue(setCookie.contains("Path=/"));
    Assertions.assertTrue(setCookie.contains("SameSite=Strict"));
  ...
  void login_shouldReturnForbiddenAndSetVerificationCookie_whenEmailIsNotVerifiedWithSession()
    ...
    Assertions.assertTrue(
        setCookie.contains("PLATFORMCORE_VERIF_TEST=11111111-2222-4333-8444-555555555555"));
  ...
  void register_shouldReturnAcceptedAndSetVerificationCookie_whenUseCaseIssuesSession()
    ...
    Assertions.assertNotNull(setCookie);
    Assertions.assertTrue(
        setCookie.contains("PLATFORMCORE_VERIF_TEST=bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"));
  ...
  void verifyEmail_shouldReturnAcceptedGenericResponse_whenVerificationCookieIsMalformed()
    ...
    mockMvc
        .perform(
            post("/v1/users/verify-email")
                .cookie(new Cookie("PLATFORMCORE_VERIF_TEST", "not-a-uuid"))
                .contentType("application/json")
                .content(
                    """
                        {"code":"123456"}
  ...
  void verifyEmail_shouldReturnNoContentAndClearVerificationCookie_whenCodeIsValid()
    ...
    var response =
        mockMvc
            .perform(
                post("/v1/users/verify-email")
                    .cookie(new Cookie("PLATFORMCORE_VERIF_TEST", verificationSessionId.toString()))
                    .contentType("application/json")
                    .content(
                        """
                            {"code":"123456"}
    ...
    String setCookie = response.getHeader(HttpHeaders.SET_COOKIE);
    Assertions.assertNotNull(setCookie);
    Assertions.assertTrue(setCookie.contains("PLATFORMCORE_VERIF_TEST="));
    Assertions.assertTrue(setCookie.contains("Max-Age=0"));
    Assertions.assertTrue(setCookie.contains("SameSite=None"));
  }
  ...
  void resendVerification_shouldReturnAcceptedGenericResponseAndSetCookie_whenSessionIsIssued()
    ...
    Assertions.assertNotNull(setCookie);
    Assertions.assertTrue(
        setCookie.contains("PLATFORMCORE_VERIF_TEST=dddddddd-dddd-4ddd-8ddd-dddddddddddd"));
