...
export class UserFormComponent implements OnInit {
  ...
  readonly form = this.formBuilder.group(
    {
      ...
      roles: [[QOMO_ROLES.USER] as string[], [rolesRequiredValidator]],
  ...
  protected readonly roleOptions = computed<RoleOption[]>(() => {
    const options: RoleOption[] = [
      {
        label: QOMO_ROLES.USER,
        value: QOMO_ROLES.USER,
      },
    ];

    if (this.sessionStore.currentUser()?.roles.includes(QOMO_ROLES.SUPERADMIN)) {
      options.push({
        label: QOMO_ROLES.ADMIN,
        value: QOMO_ROLES.ADMIN,
      });
    }
  ...
  private configureCreateMode(): void {
    ...
    this.form.controls.roles.setValue([QOMO_ROLES.USER]);
