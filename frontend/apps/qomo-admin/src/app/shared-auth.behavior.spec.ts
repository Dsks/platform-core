...
import { buildApiUrl, isApiUrl, normalizeApiBaseUrl } from '@qomo/shared-api';
import {
  adminGuard,
  authGuard,
...
describe('shared behavior used by qomo-admin', () => {
  ...
  describe('api URL helpers', () => {
    it('normalizes API base URLs', () => {
      ...
      expect(normalizeApiBaseUrl('https://api.qomo.app/v1/')).toBe(
    ...
    it('builds API URLs from normalized bases and relative paths', () => {
      ...
      expect(buildApiUrl('  users  ', 'https://api.qomo.app/v1/')).toBe(
        'https://api.qomo.app/v1/users',
      );
    ...
    it('detects URLs that belong to the configured API base', () => {
      ...
      expect(
        isApiUrl(
          'https://api.qomo.app/v1/auth/login',
          'https://api.qomo.app/v1',
        ),
      ).toBe(true);
      expect(
