...
test.describe('qomo-admin smoke tests', () => {
  ...
  test('renders the authenticated users list with the initial query', async ({
    ...
    await expect(
      page.getByRole('cell', { name: 'ana@qomo.test' }),
    ).toBeVisible();
    await expect(
      page.getByRole('cell', { name: 'beto@qomo.test' }),
    ).toBeVisible();
  ...
  test('renders the authenticated edit user form', async ({ page }) => {
    ...
    await expect(page.getByLabel(/^email$/i)).toHaveValue('ana@qomo.test');
...
async function mockApi(page: Page, options: ApiMockOptions): Promise<void> {
  ...
  await page.route('**/v1/**', async (route) => {
    ...
    await fulfillJson(route, 404, {
      title: 'Unexpected API call in qomo-admin e2e smoke test',
